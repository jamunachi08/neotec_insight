...
The transaction-history import (v2.87.3) needs Lines to already exist before anything can match against them — exactly the wall the customer's own screenshot showed: 2,209 rows, 0 matched, because none of the 33 categories had been created yet. This import removes that wall directly: bring in the Line×Month Budget/Actual statement the customer already maintains by hand, and it creates every Line that doesn't exist yet, plus writes every Budget figure found — in one pass, before the transaction import ever needs to run.

**`parse_statement_template_sheet`** verified against the customer's *real* uploaded template, not just a synthetic fixture: correctly found all 29 Cash Out + 10 Cash In lines, correctly skipped the interspersed Q1–Q4/Total subtotal columns (recognized by their header text not matching any 3-letter month prefix — "Q1"[:3] is "q1", never a real month, so no special-casing of exact subtotal labels was needed), and correctly guessed fiscal year 2026 from a real date cell above the header.

**Two real bugs caught before shipping, both found by actually checking rather than assuming:**
- A section-name bug — the *first* section header's own label was never captured as the starting section, so every line before the second section reported the wrong one. The existing class-declaration-swallowing mistake from earlier this session also recurred here (`TestMatchLinesToRows` lost its declaration line during editing) — caught by the same class-attribution-by-count discipline established after it first happened.
- A more serious one: the first draft used field names that don't exist on `Insight Cash Flow Budget` (`month`/`amount` instead of the real `period_month`/`budget_amount`) and skipped the fiscal-year-position conversion entirely — which would have silently misfiled every budget cell for any non-January-start company, the exact "right month, wrong year" bug class this app has already been bitten by once. Caught by checking the real doctype schema and matching the already-proven `save_budget_grid` pattern instead of trusting a first draft.

**Downloadable samples for both import paths** (`download_sample_history_template`, `download_sample_statement_template`), following this app's own established template-download pattern (`api/budget_import.py`). Both round-trip-verified by actually feeding the generated bytes back through the real parsers rather than assuming the generator is correct — this caught a third real gap: the statement sample's first draft had no real date cell, so `fiscal_year_guess` silently came back `None` even though the real customer template always has one. Fixed to include one.

**Frontend:** the Import modal now has a mode toggle between "Transaction history" and "Lines + Budget statement," with a "Download sample" link for whichever mode is active, and branched preview/result displays matching each import's own shape.

13 new engine tests, 240 backend tests total, all green. Frontend typechecks clean and builds — `CashFlowForecastTab` now renders as its own 36kB chunk.

**Not built this release, said plainly rather than implied:** formula-line support and the broader "sections, source, formulas — as done for consolidated P&L" request. Investigated `utils/formula.py`'s AST-based safe-eval design and have a clear plan to mirror it standalone for Cash Flow Forecast, but nothing is built yet — a genuinely separate, substantial capability that deserves its own dedicated pass rather than a rushed partial version bundled into this one.
